...
Lazy properties are intended to be passed around and only evaluated when required (usually, during the execution phase, see <<sec:build_phases>> for more on build phases), though this means they are usually evaluated more than once.
